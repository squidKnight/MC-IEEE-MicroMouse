...
INPUTS: short int pathList[NODES / 2], short int smoothList[NODES][0]
	...
	smoothList: blank array to be filled by smootherV1() with a more granular version of pathList using actual coords. smoothList[i][0] == x-coord, smoothList[i][1] == y-coord.
RETURNS: short int pathList[NODES / 2], short int smoothList[NODES][2]
	pathList: should return unmodified.
	smoothList: directly modified by smootherV1(), provides a more granular version of pathList with smoothed movements in a grid.
NOTES:
	Implemented to have smoothList array created beforehand, and assumes it to be blank and initializes it.
	The smoothList array that actively breaks the nodeID up into its component x-y parts, because the grid has more spaces than can be fit in a positive signed short int without stack overflow.
...
void smootherV1(short int pathList[NODES / 2], short int smoothList[NODES][2])
{
	...
	case -1:
		...
		fprintf(stderr, "smoothList[0] %d, smoothList[%d] %d\n", smoothList[0], 1, smoothList[1]);
		fflush(stderr);
		break;
	}
	...
	while (change >= 0.000001)
	{
		...
		for (int i = 1; i < size; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				...
				fprintf(stderr, "Checking (%d, %d) as (%d, %d)\n", smoothList[i][0], smoothList[i][1], tempList[i][0], tempList[1]);
				fflush(stderr);
				tempList[i][j] = y_i;
	...
	for (int y = 576; y > -1; y--)
	{
		for (int x = 0; x < 577; x++)
		{
			...
			/*if ((!existsP && !existsS) && !existsW)
				for (int k = NODES / 2; k < NODES; k++)
				{
					if (getXtrans(k) - (RATIO / 2) == x && getYtrans(k) - (RATIO / 2) == y)
					{
						existsW = true;
